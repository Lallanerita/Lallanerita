...
async def login(request: LoginRequest, db: aiosqlite.Connection = Depends(get_db)):
    ...
    return TokenResponse(
        ...
        token_type="bearer",
        user=UserResponse(id=user['id'], email=user['email'], name=user['name'], phone=user['phone'], address=user['address'], role=user['role'])
    )
...
async def register(request: RegisterRequest, db: aiosqlite.Connection = Depends(get_db)):
    ...
    cursor = await db.execute(
        "INSERT INTO users (email, password_hash, name, phone, address, role) VALUES (?, ?, ?, ?, ?, 'buyer')",
    ...
    return TokenResponse(
        access_token=access_token,
        token_type="bearer",
        user=UserResponse(id=user_id, email=request.email, name=request.name, phone=request.phone, address=request.address, role="buyer")
    )


@router.get("/me", response_model=UserResponse)
async def get_me(current_user: dict = Depends(get_current_user)):
    return UserResponse(id=current_user['id'], email=current_user['email'], name=current_user['name'], phone=current_user['phone'], address=current_user['address'], role=current_user['role'])


@router.put("/change-password")
async def change_password(request: ChangePasswordRequest, current_user: dict = Depends(get_current_user), db: aiosqlite.Connection = Depends(get_db)):
    ...
    await db.execute("UPDATE users SET password_hash = ? WHERE id = ?", (new_hash, current_user['id']))
    await db.commit()
