  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="sticky top-0 z-10 bg-gray-950 border-b border-gray-800 px-4 py-3">
        ...
        <div className="mx-auto max-w-6xl flex gap-2 mt-3 overflow-x-auto pb-1">
          <Button
            size="sm"
            variant={selectedCategory === null ? "default" : "outline"}
            className={selectedCategory === null ? "bg-amber-500 text-black" : "border-gray-700 text-gray-300"}
          ...
          {categories.map((c) => (
            <Button
              ...
              className={selectedCategory === c.id ? "bg-amber-500 text-black" : "border-gray-700 text-gray-300"}
            ...
            {products.map((p) => (
              <Card key={p.id} className="bg-gray-900 border-gray-800 overflow-hidden">
                ...
                <CardContent className="p-3">
                  ...
                  {p.category_name && <p className="text-xs text-amber-400">{p.category_name}</p>}
                  <div className="mt-1">
                    {p.discount_percent ? (
                      <div className="flex items-center gap-2">
                        <span className="text-sm line-through text-gray-500">{formatCOP(p.price)}</span>
                        <span className="text-sm font-bold text-amber-400">{formatCOP(p.final_price)}</span>
                      </div>
                    ) : (
                      <span className="text-sm font-bold text-amber-400">{formatCOP(p.price)}</span>
                    )}
                    <span className="text-xs text-gray-500 ml-1">/ {p.unit}</span>
                  </div>
                  <div className="flex items-center gap-1 mt-2">
                    <Button size="sm" variant="outline" className="h-7 w-7 p-0 border-gray-700" onClick={() => setQty(p.id, Math.max(p.min_order, getQty(p) - 1))}>
                      <Minus className="h-3 w-3" />
                    </Button>
                    <Input
                      type="number"
                      className="h-7 w-12 text-center text-xs bg-gray-800 border-gray-700 text-white p-0"
                      value={getQty(p)}
                      onChange={(e) => setQty(p.id, Math.max(p.min_order, Number(e.target.value)))}
                    />
                    <Button size="sm" variant="outline" className="h-7 w-7 p-0 border-gray-700" onClick={() => setQty(p.id, getQty(p) + 1)}>
                      <Plus className="h-3 w-3" />
                    </Button>
                  </div>
                  <Button
                    size="sm"
                    className={`w-full mt-2 font-bold text-xs ${inCart(p.id) ? "bg-green-700 hover:bg-green-800" : "bg-amber-500 hover:bg-amber-600 text-black"}`}
                    onClick={() => handleAdd(p)}
                  >
